"""Path Foundation pathology encoder (Google HAI-DEF).

Reference:
    Google Health AI Developer Foundations, "Path Foundation", 2024.
    https://research.google/blog/health-specific-embedding-tools-for-dermatology-and-pathology/

Path Foundation uses a ViT-S/16 architecture (``embed_dim=384``,
``patch_size=16``) trained with Masked Siamese Networks (MSN) across
magnifications on histopathology images.  The official weights are hosted
at ``google/path-foundation`` on HuggingFace Hub.

``pretrained=True`` auto-downloads from HF Hub.
``pretrained=False`` raises ``RuntimeError``.

Registered as ``"path_foundation"`` in ``ENCODER_REGISTRY``.
"""
# Source: https://huggingface.co/google/path-foundation
...
_PRIMARY_HF_NAME = "google/path-foundation"
PRIMARY_BACKBONE_NAME = _PRIMARY_HF_NAME
_EMBED_DIM = 384
_PATCH_SIZE = 16


@ENCODER_REGISTRY.register("path_foundation")
class PathFoundationEncoder(BaseFoundationEncoder):
    """Path Foundation (pathology ViT-S/16, MSN) encoder with DPT-style multi-block pyramid.

    The backbone is a ViT-Small/16 (``embed_dim=384``, ``patch_size=16``)
    pretrained with Masked Siamese Networks on histopathology images.
    ...
    def __init__(self, in_channels: int = 3, img_size: Optional[int] = None,
        ...
        if pretrained:
            try:
                ...
                # Fallback: build ViT-S/16 skeleton and download weights.
                import transformers
                ...
                warnings.warn(f"[path_foundation] loaded via hf_hub_download: {_msg}")
                self.backbone = HuggingFaceViTWrapper(_skel)
        else:
            raise RuntimeError(
                "PathFoundationEncoder does not support pretrained=False. "
                "This encoder requires pretrained weights from "
                "'google/path-foundation'. Pass pretrained=True to "

...
Phikon is an iBOT-style self-supervised ViT-B/16 (``patch_size=16``,
``embed_dim=768``) pretrained on TCGA whole-slide image tiles. The official
weights are hosted at ``owkin/phikon`` on HuggingFace Hub (a transformers
``ViTModel`` checkpoint). ``pretrained=True`` auto-downloads from HF Hub.
``pretrained=False`` raises ``RuntimeError``.

The ViT token grid is reshaped to a ``(B, dim, h, w)`` feature map and
projected into a four-stage DPT-style multi-block pyramid (deepest LAST), matching
the ``BaseFoundationEncoder`` contract used by the rest of the foundation
family.
"""
# Source: https://huggingface.co/owkin/phikon
...
# Phikon official release on the Hugging Face hub.
_PRIMARY_HF_NAME = "owkin/phikon"
...
class PhikonEncoder(BaseFoundationEncoder):
    ...
    ``(B, 768, h, w)`` and projected by four DPT-style multi-block stages into a
    ...
    * stage 3 - 2x down -> 1x1 conv to ``dim``   channels   (~ H/32)
    ...
    def __init__(self, in_channels: int = 3, img_size: Optional[int] = None,
        ...
        if pretrained:
            ...
            raise RuntimeError(
                "PhikonEncoder does not support pretrained=False. "
                "This encoder requires pretrained weights from 'owkin/phikon'. "

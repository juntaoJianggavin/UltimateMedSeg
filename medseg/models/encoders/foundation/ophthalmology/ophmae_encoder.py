...
Reference:
    ...
    for Adaptive Ophthalmological Diagnosis, npj Digital Medicine, 2025.

OphMAE is a multi-imaging MAE-style ViT-Base/16 (``embed_dim=768``,
``patch_size=16``) pretrained on retinal OCT and IR images.  The official
weights are hosted at ``OphMAE/OphMAE_ckpt`` on HuggingFace Hub.

``pretrained=True`` auto-downloads from HF Hub.
``pretrained=False`` raises ``RuntimeError``.

Registered as ``"ophmae"`` in ``ENCODER_REGISTRY``.
"""
# Source: https://huggingface.co/OphMAE/OphMAE_ckpt
# Paper:  https://www.nature.com/articles/s41746-025-01852-3
...
_PRIMARY_HF_NAME = "OphMAE/OphMAE_ckpt"
    ...
    def __init__(self, in_channels: int = 3, img_size: Optional[int] = None,
        ...
        if pretrained:
            try:
                ...
                warnings.warn(f"[ophmae] loaded via hf_hub_download: {_msg}")
                self.backbone = HuggingFaceViTWrapper(_skel)
        else:
            raise RuntimeError(
                "OphMAEEncoder does not support pretrained=False. "
                "This encoder requires pretrained weights from "
                "'OphMAE/OphMAE_ckpt'. Pass pretrained=True to "

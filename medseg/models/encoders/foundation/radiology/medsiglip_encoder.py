...
Reference:
    ...
    Classification", 2025.  Model card: ``google/medsiglip-448``.

MedSigLIP is a 400M-parameter SigLIP-style vision encoder (So400m/14,
``embed_dim=1152``, ``patch_size=14``, native resolution 448×448)
pre-trained on large-scale medical imaging data.  The vision tower is
loaded directly via ``transformers.SiglipVisionModel``.

``pretrained=True`` auto-downloads from ``google/medsiglip-448`` (gated —
accept the licence on HF first).  ``pretrained=False`` raises RuntimeError.

Output:
    forward(x) -> List[Tensor] of length 4 with channels
    [dim/8, dim/4, dim/2, dim] (deepest LAST).

Registered as ``"medsiglip"`` in ``ENCODER_REGISTRY``.
"""
...
_PRIMARY_HF_NAME = "google/medsiglip-448"
...
class MedSigLIPEncoder(BaseFoundationEncoder):
    ...
    ``(B, 1152, h, w)`` and projected by four DPT-style multi-block stages into a
    multi-scale pyramid (deepest LAST):
    ...
    * stage 3 — 2x down -> 1x1 conv to ``dim``   channels  (~ H/32)
    ...
    Args:
        ...
        img_size: Reference spatial size (default 448).
    ...
    def __init__(self, in_channels: int = 3, img_size: Optional[int] = None,
        ...
        if pretrained:
            self.backbone = load_hf_vit(
                ...
                model_cls_name="SiglipVisionModel",
            )
        else:
            raise RuntimeError(
                "MedSigLIPEncoder does not support pretrained=False. "
                "This encoder requires weights from 'google/medsiglip-448'. "
                "Accept the licence on HF Hub, then pass pretrained=True to "

...
the vision encoder for use as a segmentation backbone, so this module wraps
it via ``transformers.AutoModel`` and exposes a 4-stage multi-scale pyramid
via the SAM-style "DPT-style multi-block" trick.

``pretrained=True`` auto-downloads from ``google/medgemma-4b-pt`` (gated repo
...
Output:
    ...
    (deepest LAST).
"""
# Source: https://huggingface.co/google/medgemma-4b-pt
...
_PRIMARY_HF_NAME = "google/medgemma-4b-pt"
    ...
    def __init__(self, in_channels: int = 3, img_size: Optional[int] = None,
        ...
        if pretrained:
            ...
            try:
                ...
                raise RuntimeError(
                    f"MedGemma auto-download from '{_PRIMARY_HF_NAME}' failed: "
                    f"{type(e).__name__}: {e}. Accept the licence on HF and "
            ...
            if _visual is None:
                raise RuntimeError(
                    f"Could not find vision tower in MedGemma model from '{_src}'."
                )
            ...
            raise RuntimeError(
                "MedGemmaVisionEncoder does not support pretrained=False. "
                "This encoder requires MedGemma vision tower weights from "

"""Lingshu (灵枢) MLLM vision-encoder backbone.

Lingshu (Alibaba DAMO Academy, 2025) is a generalist medical multimodal
LLM built on top of the Qwen2.5-VL architecture. The vision tower is
the Qwen2.5-VL ViT: ``hidden_size=1280``, ``depth=32``, ``patch_size=14``,
``spatial_merge_size=2``.

``pretrained=True`` auto-downloads from ``lingshu-medical-mllm/Lingshu-7B``
...
Output:
    ...
    (deepest LAST), dim=1280.
"""
# Source: https://huggingface.co/lingshu-medical-mllm/Lingshu-7B
...
_PRIMARY_HF_NAME = "lingshu-medical-mllm/Lingshu-7B"
    ...
    def __init__(self, in_channels: int = 3, img_size: Optional[int] = None,
        ...
        if pretrained:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Lingshu auto-download from '{_PRIMARY_HF_NAME}' failed: "
            ...
            if _visual is None:
                raise RuntimeError(
                    f"Could not find vision tower in Lingshu model from '{_src}'."
                )
            ...
            raise RuntimeError(
                "LingshuVisionEncoder does not support pretrained=False. "
                "This encoder requires Lingshu vision tower weights from "

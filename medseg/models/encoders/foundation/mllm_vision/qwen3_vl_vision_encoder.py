...
M-RoPE (multimodal rotary positional embeddings) and dynamic-resolution
patchification.

``pretrained=True`` auto-downloads from ``Qwen/Qwen3-VL-8B-Instruct``
...
_PRIMARY_HF_NAME = "Qwen/Qwen3-VL-8B-Instruct"
    ...
    def __init__(self, in_channels: int = 3, img_size: Optional[int] = None,
        ...
        if pretrained:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Qwen3-VL auto-download from '{_PRIMARY_HF_NAME}' failed: "
            ...
            if _visual is None:
                raise RuntimeError(
                    f"Could not find vision tower in Qwen3-VL model from '{_src}'."
                )
            ...
            raise RuntimeError(
                "Qwen3VLVisionEncoder does not support pretrained=False. "
                "This encoder requires Qwen3-VL vision tower weights from "

"""LLaVA-Med vision encoder (foundation-model encoder).

LLaVA-Med (Microsoft, 2023) is a medical multimodal LLM. Its vision tower is
a CLIP ViT-L/14 (embed_dim=1024, depth=24, patch_size=14) operating at a
native input resolution of 336x336 — identical in structure to OpenAI's
CLIP ViT-L/14-336, just (further-)tuned on biomedical figure-caption data.

``pretrained=True`` auto-downloads from ``openai/clip-vit-large-patch14-336``
via ``transformers.CLIPVisionModel``. ``pretrained=False`` raises RuntimeError.
...
@ENCODER_REGISTRY.register("llava_med_vision")
class LLaVAMedVisionEncoder(BaseFoundationEncoder):
    """LLaVA-Med vision-tower encoder with DPT-style multi-block multi-scale output.

    Constructor follows the BaseFoundationEncoder contract. The LLaVA-Med
    ...
    def __init__(self, in_channels: int = 3, img_size: Optional[int] = None,
        ...
        if pretrained:
            ...
            raise RuntimeError(
                "LLaVAMedVisionEncoder does not support pretrained=False. "

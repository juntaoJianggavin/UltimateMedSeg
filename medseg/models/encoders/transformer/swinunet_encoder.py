    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.swin = SwinTransformerSys(
            ...
            window_size=window_size, patch_norm=True, use_checkpoint=False)

        self._out_channels = [int(embed_dim * 2 ** i) for i in range(len(depths))]
    ...
    def _load_pretrained(self, path):
        ...
        for k, v in state.items():
            if k.startswith('layers_up') or k.startswith('concat_back_dim') or \
               k.startswith('norm_up') or k.startswith('up.') or k.startswith('output.'):
                continue
            encoder_state[k] = v
        msg = self.swin.load_state_dict(encoder_state, strict=False)
        print(f"SwinUNet encoder loaded: {msg}")

    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        _, x_downsample = self.swin(x)
        patches_resolution = self.swin.patches_resolution
        B = x.shape[0]

        features = []
        for i, feat in enumerate(x_downsample):
            H = patches_resolution[0] // (2 ** i)
            W = patches_resolution[1] // (2 ** i)
            C = self._out_channels[i]
            feat_2d = feat.view(B, H, W, C).permute(0, 3, 1, 2).contiguous()
